...
function getPriority(c) {
  if (c.Status === 'Lapsed') return { score:95, label:'CRITICAL', sub:'Lapsed', color:'#e87e7e', bg:'#1a0808' }
  const diff = dayDiff(c.Next_Reminder_Date)
  if (diff === null) return { score:10, label:'LOW', sub:'No date', color:'#9ca3af', bg:'transparent' }
  if (diff < -2) return { score:90+Math.min(Math.abs(diff),9), label:'CRITICAL', sub:`${Math.abs(diff)}d overdue`, color:'#ef4444', bg:'transparent' }
  if (diff < 0)  return { score:75, label:'HIGH', sub:`${Math.abs(diff)}d overdue`, color:'#f97316', bg:'transparent' }
  if (diff === 0) return { score:70, label:'DUE TODAY', sub:'Act now', color:'#eab308', bg:'transparent' }
  if (diff <= 3)  return { score:50, label:'HIGH', sub:`In ${diff}d`, color:'#3b82f6', bg:'transparent' }
  return { score:20, label:'MED', sub:`In ${diff}d`, color:'#6b7280', bg:'transparent' }
}
...
function Toast({msg, type, onDone}) {
  ...
  const ok = type !== 'error'
  return (
    <div style={{position:'fixed',top:20,right:20,zIndex:1000,background:ok?'#dcfce7':'#fee2e2',border:`1px solid ${ok?'#86efac':'#fca5a5'}`,color:ok?'#166534':'#991b1b',padding:'12px 18px',borderRadius:8,fontSize:13,fontWeight:500,boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}}>
      {ok?'✓':'✗'} {msg}
    </div>
  )
}
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div style={{background:'#fff',borderRadius:14,width:'100%',maxWidth:480,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 8px 40px rgba(0,0,0,0.15)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 20px',borderBottom:'0.5px solid #e5e7eb'}}>
          <div>
            <div style={{fontSize:16,fontWeight:600,color:'#111'}}>Add Client</div>
            <div style={{fontSize:11,color:'#6b7280',marginTop:2}}>Fill in details to add to follow-up system</div>
          </div>
        ...
        <div style={{padding:20}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={submit} disabled={saving} style={{flex:2,background:saving?'#93c5fd':'#2563eb',border:'none',color:'#fff',padding:10,borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:500}}>
...
export default function Dashboard() {
  ...
  async function sendReminder(c,e) {
    ...
    try {
      await fetch(REMIND_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({client_id:c.Client_ID,row_number:c.row_number,name:c.Full_Name,email:c.Email,treatment:c.Treatment_Type})})
  ...
  async function markDone(c,e) {
    ...
    } catch { showToast('Failed to mark done','error') }
  ...
  const filtered = sorted.filter(c=>{
    ...
    const ss = fStage==='All'||c.Reminder_Stage===fStage
    const ts = fStatus==='All'||c.Status===fStatus
    return ms&&ss&&ts
  })
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:'#f5f6f7',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{width:160,flexShrink:0,background:'#fff',borderRight:'0.5px solid #e5e7eb',padding:'20px 0',display:'flex',flexDirection:'column'}}>
        <div style={{fontSize:15,fontWeight:600,color:'#111',padding:'0 16px 16px',borderBottom:'0.5px solid #e5e7eb',marginBottom:12}}>ClinicPulse</div>
        {navItems.map(n=>(
          <div key={n} onClick={()=>setActiveNav(n)} style={{padding:'8px 16px',fontSize:13,cursor:'pointer',color:activeNav===n?'#1d4ed8':'#6b7280',background:activeNav===n?'#eff6ff':'transparent',borderRight:activeNav===n?'2px solid #3b82f6':'2px solid transparent',fontWeight:activeNav===n?500:400}}>
            {n}
          </div>
        ))}
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:24}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:20}}>
          <div>
            <div style={{fontSize:24,fontWeight:600,color:'#111'}}>Clinic Command Center</div>
          ...
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <select style={{fontSize:12,padding:'6px 10px',border:'0.5px solid #d1d5db',borderRadius:8,background:'#fff',color:'#374151'}}><option>All Clinics</option></select>
            <select style={{fontSize:12,padding:'6px 10px',border:'0.5px solid #d1d5db',borderRadius:8,background:'#fff',color:'#374151'}}><option>Last 30 days</option><option>Last 12 weeks</option></select>
        ...
        {/* Alert */}
        {topClient && (
          <div style={{background:'#fffbeb',border:'0.5px solid #fcd34d',borderRadius:8,padding:'10px 16px',marginBottom:16,fontSize:13,color:'#92400e',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span>⚡ Priority: <strong>{topClient.Full_Name}</strong> is {getPriority(topClient).sub}. Send reminder now to recover <strong>AED {getRevenue(topClient).toLocaleString()}</strong> today.</span>
          </div>
        )}
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
          {[
            {label:'Revenue at Risk',   value:`AED ${Math.round(totalRisk).toLocaleString()}`,  color:'#1d4ed8', bars:['#bfdbfe','#93c5fd','#3b82f6','#1d4ed8','#1e3a8a']},
            {label:'Recovered Revenue', value:`AED ${Math.round(recovered).toLocaleString()}`,  color:'#15803d', bars:['#bbf7d0','#86efac','#22c55e','#15803d','#14532d']},
            {label:'Lost Revenue',      value:`AED ${Math.round(lost).toLocaleString()}`,        color:'#b91c1c', bars:['#fecaca','#fca5a5','#ef4444','#b91c1c','#7f1d1d']},
            {label:'Action Queue',      value:actionQueue,                                        color:'#1d4ed8', bars:['#bfdbfe','#93c5fd','#3b82f6','#1d4ed8','#1e3a8a']},
          ].map(m=>(
            <div key={m.label} style={{background:'#fff',border:'0.5px solid #e5e7eb',borderRadius:12,padding:'14px 16px'}}>
              ...
              <div style={{display:'flex',gap:2,marginTop:8,height:4}}>
                {m.bars.map((b,i)=><span key={i} style={{flex:1,background:b,borderRadius:2}}/>)}
              </div>
            </div>
          ))}
        </div>
              ...
              {[{label:'WhatsApp',pct:72,color:'#3b82f6'},{label:'Call',pct:64,color:'#22c55e'},{label:'SMS',pct:55,color:'#8b5cf6'},{label:'Email',pct:39,color:'#6b7280'}].map(ch=>(
                <div key={ch.label}>
                  <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:3}}>
                    <span style={{color:'#374151'}}>{ch.label}</span>
                    <span style={{color:ch.color,fontWeight:500}}>{ch.pct}%</span>
                  </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:20}}>
          <div style={{background:'#fff',border:'0.5px solid #e5e7eb',borderRadius:12,padding:16}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {[
                {label:'Lead',       count:clients.length,                          pct:100, color:'#bfdbfe', tc:'#1e3a8a'},
                {label:'Appointment',count:Math.round(clients.length*0.82),         pct:82,  color:'#3b82f6', tc:'#fff'},
                {label:'Visit',      count:Math.round(clients.length*0.65),         pct:65,  color:'#1d4ed8', tc:'#fff'},
                {label:'Revenue',    count:`AED ${Math.round(totalRisk*0.4).toLocaleString()}`, pct:40, color:'#1e3a8a', tc:'#fff'},
              ].map(f=>(
        ...
        <div style={{background:'#fff',border:'0.5px solid #e5e7eb',borderRadius:12,padding:16}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{fontSize:12,padding:'6px 10px',border:'0.5px solid #d1d5db',borderRadius:8,background:'#fff',color:'#374151',width:160,outline:'none'}}/>
              <select value={fStage} onChange={e=>setFStage(e.target.value)} style={{fontSize:12,padding:'6px 10px',border:'0.5px solid #d1d5db',borderRadius:8,background:'#fff',color:'#374151'}}>
                <option>All</option>{STAGES.map(s=><option key={s}>{s}</option>)}
              </select>
              <select value={fStatus} onChange={e=>setFStatus(e.target.value)} style={{fontSize:12,padding:'6px 10px',border:'0.5px solid #d1d5db',borderRadius:8,background:'#fff',color:'#374151'}}>
                <option>All</option><option>Active</option><option>Lapsed</option>
              </select>
            </div>
          </div>
          {loading ? (
            <div style={{textAlign:'center',padding:40,color:'#6b7280'}}>Loading clients...</div>
          ) : error ? (
            <div style={{textAlign:'center',padding:40,color:'#ef4444'}}>Could not load data: {error}</div>
          ) : (
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
              <thead>
                <tr>
                  {['Patient','Priority','Next Action','Stage','Follow-up','Revenue','Quick Actions'].map(h=>(
                    <th key={h} style={{fontSize:11,color:'#6b7280',textAlign:'left',padding:'6px 10px',borderBottom:'0.5px solid #e5e7eb',fontWeight:400}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((c,i)=>{
                  ...
                  const cid   = c.Client_ID||`row-${i}`
                  const isEdit= editRow===cid
                  const td    = {padding:'10px 10px',borderBottom:'0.5px solid #e5e7eb',color:'#111',verticalAlign:'middle'}
                  return (
                    <tr key={cid} onClick={()=>{ if(!isEdit){setSelected(c===selected?null:c);setEditRow(null)} }} style={{cursor:'pointer',background:selected===c?'#f8fafc':'#fff'}}>
                      <td style={td}>
                        <div style={{fontWeight:500}}>{c.Full_Name}</div>
                        <div style={{fontSize:10,color:'#9ca3af'}}>· {c.Client_ID}</div>
                      </td>
                      <td style={td}>
                        <div style={{fontSize:11,fontWeight:700,color:pri.color}}>{pri.label}</div>
                        <div style={{fontSize:10,color:'#9ca3af'}}>{pri.sub}</div>
                      </td>
                      <td style={{...td,fontSize:12,color:'#6b7280',maxWidth:180}}>{getAIMove(c)}</td>
                      <td style={td}>
                        {isEdit ? (
                          <select value={editV.Reminder_Stage} onClick={e=>e.stopPropagation()} onChange={e=>setEditV(v=>({...v,Reminder_Stage:e.target.value}))} style={{background:'#f9fafb',border:'0.5px solid #d1d5db',borderRadius:4,color:'#111',fontSize:11,padding:'3px 5px',outline:'none'}}>
                            {STAGES.map(s=><option key={s}>{s}</option>)}
                          </select>
                        ) : (
                          <span style={{background:stage.bg,color:stage.color,padding:'3px 8px',borderRadius:4,fontSize:10,fontWeight:500,whiteSpace:'nowrap'}}>{c.Reminder_Stage}</span>
                        )}
                      </td>
                      <td style={td}>
                        {isEdit ? (
                          ...
                          <div>
                            <div style={{fontSize:11,color:diff!==null&&diff<0?'#ef4444':diff===0?'#22c55e':'#6b7280',fontWeight:diff!==null&&diff<=0?600:400}}>
                              {diff===null?'Not set':diff===0?'Today · Now':diff<0?`Overdue · ${Math.abs(diff)}d`:`In ${diff}d`}
                            </div>
                            {c.Next_Reminder_Date&&<div style={{fontSize:10,color:'#9ca3af'}}>{c.Next_Reminder_Date}</div>}
                          </div>
                        )}
                      </td>
                      ...
                      <td style={td}>
                        {isEdit ? (
                          <div style={{display:'flex',gap:4}}>
                            <button onClick={e=>saveEdit(c,e)} style={{fontSize:11,padding:'4px 8px',border:'none',borderRadius:6,background:'#dcfce7',color:'#15803d',cursor:'pointer'}}>Save</button>
                            <button onClick={e=>{e.stopPropagation();setEditRow(null)}} style={{fontSize:11,padding:'4px 8px',border:'0.5px solid #d1d5db',borderRadius:6,background:'#fff',color:'#6b7280',cursor:'pointer'}}>×</button>
                          </div>
                        ) : (
                          <div style={{display:'flex',gap:4}}>
                            <button onClick={e=>sendReminder(c,e)} disabled={sending===c.Client_ID} style={{fontSize:11,padding:'4px 8px',border:'none',borderRadius:6,background:'#eff6ff',color:'#1d4ed8',cursor:'pointer',opacity:sending===c.Client_ID?0.5:1}}>
                              {sending===c.Client_ID?'...':'+ WA'}
                            </button>
                            <button onClick={e=>markDone(c,e)} disabled={doing===c.Client_ID} style={{fontSize:11,padding:'4px 8px',border:'none',borderRadius:6,background:'#f0fdf4',color:'#15803d',cursor:'pointer',opacity:doing===c.Client_ID?0.5:1}}>
                              {doing===c.Client_ID?'...':'✓'}
                            </button>
                            ...
                            <button onClick={e=>{e.stopPropagation();setSelected(c===selected?null:c)}} style={{fontSize:11,padding:'4px 8px',border:'0.5px solid #d1d5db',borderRadius:6,background:'#fff',color:'#6b7280',cursor:'pointer'}}>→</button>
                          </div>
                        )}
                      </td>
                    </tr>
          ...
          <div style={{padding:'8px 10px',borderTop:'0.5px solid #e5e7eb',fontSize:11,color:'#9ca3af',display:'flex',justifyContent:'space-between',marginTop:4}}>
            <span>Top-priority actions are ranked by risk score and projected revenue.</span>
      ...
      {/* Side Panel */}
      {selected && (
        <div style={{width:280,flexShrink:0,background:'#fff',borderLeft:'0.5px solid #e5e7eb',padding:20,overflowY:'auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            <div style={{fontSize:15,fontWeight:600,color:'#111'}}>{selected.Full_Name}</div>
            <button onClick={()=>setSelected(null)} style={{background:'none',border:'none',cursor:'pointer',color:'#6b7280',fontSize:20}}>×</button>
          </div>
          {[['Client ID',selected.Client_ID],['Treatment',selected.Treatment_Type],['WhatsApp',selected.WhatsApp_Number],['Email',selected.Email||'—'],['Status',selected.Status],['Last Reminder',selected.Last_Reminder_Sent||'Never']].map(([k,v])=>(
          ...
          <div style={{marginBottom:12}}>
            <div style={{fontSize:11,color:'#6b7280',marginBottom:3}}>Stage</div>
            <select value={editV.Reminder_Stage||selected.Reminder_Stage} onChange={e=>{setEditV(v=>({...v,Reminder_Stage:e.target.value}));setEditRow(selected.Client_ID)}} style={{...inp,fontSize:12}}>
              {STAGES.map(s=><option key={s}>{s}</option>)}
            </select>
          </div>
          <div style={{marginBottom:16}}>
            <div style={{fontSize:11,color:'#6b7280',marginBottom:3}}>Next Follow-up</div>
            <input type="date" value={editV.Next_Reminder_Date?editV.Next_Reminder_Date.split('/').reverse().join('-'):selected.Next_Reminder_Date?selected.Next_Reminder_Date.split('/').reverse().join('-'):''} onChange={e=>{const p=e.target.value.split('-');setEditV(v=>({...v,Next_Reminder_Date:`${p[2]}/${p[1]}/${p[0]}`}));setEditRow(selected.Client_ID)}} style={{...inp,fontSize:12}}/>
          </div>
          {editRow===selected.Client_ID&&(
            <button onClick={e=>saveEdit(selected,e)} style={{width:'100%',background:'#2563eb',border:'none',color:'#fff',padding:'8px',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:500,marginBottom:12}}>Save Changes</button>
          )}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:11,color:'#6b7280',marginBottom:3}}>Notes</div>
            <div style={{fontSize:13,color:'#374151',padding:'8px 10px',background:'#f9fafb',borderRadius:6,minHeight:40}}>{selected.Notes||'—'}</div>
          </div>
          <div>
            <div style={{fontSize:11,color:'#6b7280',marginBottom:3}}>AI Recommendation</div>
            <div style={{fontSize:12,color:'#374151',padding:'8px 10px',background:'#f0fdf4',borderRadius:6,border:'0.5px solid #bbf7d0'}}>{getAIMove(selected)}</div>
          </div>
        </div>
      )}
